...
df[all_samples] = df[all_s] = df[all_samples].div(df[all_samples].sum(axis=0), axis=1)
...
for idx, row in df.iterrows():
    ...
    is_increasing = (m_hs2w >= m_sham) and (m_hs6w >= m_hs2w) and (m_hs10w >= m_hs6w)
    is_decreasing = (m_hs2w <= m_sham) and (m_hs6w <= m_hs2w) and (m_hs10w <= m_hs6w)
    
    if m_sham > 0.001 or m_hs2w > 0.001 or m_hs6w > 0.001 or m_hs10w > 0.001:
        trend = "Stable"
        if is_increasing and m_hs10w > m_sham: trend = "Increasing"
        elif is_decreasing and m_hs10w < m_sham: trend = "Decreasing"
...
print(res_df[res_df['Trend'] != 'Stable'].sort_values('Mean_HS10W', ascending=False).head(10).to_string())
