...
# Merge for unified feature set
merged = pd.merge(f1[tax_cols + f1_samples], f2[tax_cols + f2_samples], on=tax_cols, how='outer').fillna(0)

# Recalculate all_samples list (unique samples)
all_samples = f1_samples + f2_samples

# Alpha
alpha_data = []
for s in all_samples:
    counts = merged[s].values
    group = s.split('-')[0]
    if 'GaE' in s: group = 'HS10WGaE9' if 'GaE9' in s else 'HS10WGaE10'
    alpha_data.append({'Sample': s, 'Group': group, 'Shannon': calculate_shannon(counts), 'Chao1': calculate_chao1(counts)})
